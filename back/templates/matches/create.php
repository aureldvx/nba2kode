...
<header class="flex items-center justify-between p-4 shadow-md fixed w-full bg-white">
  <h1 class="text-2xl font-bold text-indigo-500">Ajouter un match</h1>
  <div class="actions">
...
<main class="pt-44">
  <form action="<?= str_replace('/index.php', '', $_SERVER['PHP_SELF']) ?>" method="post" class="ml-auto mr-auto rounded-md shadow-lg flex flex-col items-stretch w-5/12 p-5">
    <div>
      ...
      <input type="datetime-local" name="play_date" id="play_date" class="appearance-none rounded-md block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
    </div>
    <fieldset class="mt-10">
      ...
      <div class="mt-5">
        ...
        <input type="number" name="home_score" id="home_score" class="appearance-none rounded-md block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
      </div>
    </fieldset>
    <fieldset class="mt-10">
      ...
      <div class="mt-5">
        ...
        <input type="number" name="away_score" id="away_score" class="appearance-none rounded-md block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
      </div>
    </fieldset>
    <input type="hidden" name="_csrf_token" value="<?= $vars['_csrf_token'] ?>">
    <div class="mt-5 w-full">
      <button type="submit" class="appearance-none rounded-md block w-full px-3 py-2 bg-indigo-500 text-white">Créer le match</button>
    </div>
  </form>
</main>
